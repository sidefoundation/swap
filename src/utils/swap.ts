...
export class MarketMaker {
  ...
  getRatio(denom1:string, denom2:string) {
    return  this.findAssetByDenom(denom1).balance.amount.parseToFloat()/this.findAssetByDenom(denom2).balance.amount.parseToFloat()
  }
  ...
  multiDeposit(tokenIns: Coin[]):Coin[] {
    const outTokens = tokenIns.map((token)=>{
      ...
      const ratio = token.amount.parseToFloat() /  asset.balance.amount.parseToFloat()
  ...
  singleWithdraw(denomOut: string, redeem:Coin): Coin|undefined {
    ...
    const ratio = 1 - redeem.amount.parseToFloat()/this.pool.supply.amount.parseToFloat()  
  ...
  multiWithdraw(denomOut:string, redeem:Coin):Coin {
    const asset = this.findAssetByDenom(denomOut)
	  const out = asset.balance.amount.parseToFloat()*redeem.amount.parseToFloat()/this.pool.supply.amount.parseToFloat()
...
// Ai = Bi * ((Bo/(Bo - Ao)) ** Wo/Wi -1)
 rightSwap(amountIn:Coin, amountOut:Coin): Coin|undefined {
	    ...
	    const denominator = assetOut.balance.amount.parseToFloat()-amountOut.amount.parseToFloat()
