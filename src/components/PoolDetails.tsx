...
import { LocalDeposit, MsgMultiAssetDepositRequest, MsgSingleAssetDepositRequest, RemoteDeposit } from '@/codegen/ibc/applications/interchain_swap/v1/tx';
...
export function PoolDetails({ pool, onEnablePool }: PoolDetailsProps) {
  ...
  const onDoubleDeposit = async(localDenom: string, remoteDenom:string) => {
    ...
    const localDepositCoin = depositCoin.find((item)=>item.denom === localDenom)
    const remoteDepositCoin = depositCoin.find((item)=>item.denom === remoteDenom)
    ...
    const remoteAsset = pool.assets.find((item)=>item.balance.denom === remoteWallet.chainInfo.denom)
      
    const localAsset = pool.assets.find((item)=>item.balance.denom === wallet.chainInfo.denom)
    ...
    if (ratio !== (+remoteDepositCoin.amount)/(+localDepositCoin.amount)) {
      console.log("invalid pair:", ratio, "please input:",  +localDepositCoin.amount*ratio)
      return
    }
...
<div className='flex items-center justify-center gap-4 mt-4'>
      ...
      onChange={(coin) => {
        setDepositCoin(prevDepositCoin => {
          ...
          newDepositCoin[index] = {denom: item.balance.denom, amount: coin}; // Updating our copied array
    ...
    {pool.status === 'POOL_STATUS_READY' && (
          <div className='flex justify-between gap-4'>{pool.assets.map((asset)=>{
            return<div className='ml-4'>
                  <button className='btn btn-primary' onClick={()=>onSingleDeposit(asset.balance.denom)}>Withdraw "{asset.balance.denom}"</button>
                </div>
            })
          }<button className='btn btn-primary' onClick={()=>onDoubleDeposit("aside","bside")}>Multi-Withdraw</button></div>
        )}
      </div>
